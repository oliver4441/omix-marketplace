        ...
        <aside className="w-56 shrink-0 hidden md:block">
          <div className="glass-card p-4 mb-4 sticky top-20">
            ...
            <div className="space-y-0.5">
              <Link href="/" className="block px-3 py-2 rounded-lg text-sm transition-colors bg-emerald-500/15 text-emerald-400 font-medium">All Categories</Link>
              {CATEGORIES.map((c) => <Link key={c.slug} href={`/?category=${c.slug}`} className="block px-3 py-2 rounded-lg text-sm transition-colors text-slate-400 hover:text-white hover:bg-white/5">{c.name}</Link>)}
            </div>
          </div>
          <div className="glass-card p-4 sticky top-96">
            <h3 className="text-sm font-semibold text-white mb-3">Condition</h3>
            <div className="space-y-0.5">
              <Link href="/" className="block px-3 py-2 rounded-lg text-sm transition-colors bg-emerald-500/15 text-emerald-400 font-medium">All Conditions</Link>
              {CONDITIONS.map((c) => <Link key={c.value} href={`/?condition=${c.value}`} className="block px-3 py-2 rounded-lg text-sm transition-colors text-slate-400 hover:text-white hover:bg-white/5">{c.label}</Link>)}
            </div>
          </div>
        </aside>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between mb-4 gap-3 flex-wrap">
            <p className="text-sm text-slate-400">
              <span className="text-white font-medium">{total}</span> listing{total !== 1 ? "s" : ""} found
              {error && <span className="text-amber-400 ml-2">(offline mode)</span>}
            </p>
            ...
            <div className="glass-card p-12 text-center">
              {error ? (
                <>
                  ...
                  <p className="text-amber-400 text-lg font-medium mb-2">Connection Issue</p>
                  <p className="text-slate-500 text-sm mb-4">Could not connect to the database. Please check your connection and try again.</p>
                  <button onClick={() => window.location.reload()} className="glass-btn text-sm">Retry</button>
                </>
              ) : (
                <>
