...
export default function CheckoutForm({ cartItems }: { cartItems: CartItem[] }) {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      <h1 className="text-2xl font-bold mb-6 text-[#222222]">Checkout</h1>
      {step === "success" ? (
        <div className="bg-white border border-[#ebebeb] rounded-[14px] p-8 text-center">
          ...
          <p className="text-xl font-bold text-[#222222] mb-2">Order Placed!</p>
          <p className="text-[#6a6a6a] font-medium">Your order has been created successfully.</p>
          <p className="text-sm text-[#8f8f8f] mt-2">You will receive an M-Pesa STK push prompt on your phone.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
          <div className="lg:col-span-3">
            <form onSubmit={handleCheckout} className="space-y-6">
              <div className="bg-white border border-[#ebebeb] rounded-[14px] p-5">
                <h2 className="font-semibold mb-4 text-[#222222]">Delivery Information</h2>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div><label className="block text-sm font-medium text-[#222222] mb-1">Full Name</label><input required className="airbnb-input" /></div>
                  <div><label className="block text-sm font-medium text-[#222222] mb-1">M-Pesa Phone *</label><input required type="tel" placeholder="07XXXXXXXX" value={phone} onChange={(e) => setPhone(e.target.value)} className="airbnb-input" /></div>
                  <div className="col-span-2"><label className="block text-sm font-medium text-[#222222] mb-1">Pickup / Delivery Location</label><input required placeholder="e.g., Kericho Town, near university" className="airbnb-input" /></div>
                </div>
              </div>
              <div className="bg-white border border-[#ebebeb] rounded-[14px] p-5">
                <h2 className="font-semibold mb-4 text-[#222222]">Payment Method</h2>
                <label className="flex items-center gap-3 p-3 border-2 border-[#222222] bg-[#f7f7f7] rounded-xl cursor-pointer">
                  <input type="radio" name="payment" defaultChecked className="accent-[#222222]" />
                  <div className="flex-1"><p className="font-medium text-sm text-[#222222]">M-Pesa STK Push</p><p className="text-xs text-[#8f8f8f]">Pay directly from your M-Pesa account. Secure and instant.</p></div>
                </label>
              </div>
          ...
          <div className="lg:col-span-2">
            <div className="bg-white border border-[#ebebeb] rounded-[14px] p-5 sticky top-[88px]">
              <h2 className="font-semibold mb-4 text-[#222222]">Order Summary ({cartItems.length})</h2>
              <div className="space-y-3 max-h-64 overflow-y-auto">
                {cartItems.map((item) => (
                  <div key={item.cart_id} className="flex gap-3">
                    <div className="w-12 h-12 bg-[#f7f7f7] rounded-lg overflow-hidden shrink-0">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-[#8f8f8f]">{item.seller_name}</p>
                      <p className="text-sm font-semibold text-[#222222]">{formatPrice(item.price)}</p>
                    </div>
                  </div>
                ))}
              </div>
              <div className="border-t border-[#ebebeb] pt-3 mt-3 space-y-1 text-sm">
                <div className="flex justify-between text-[#6a6a6a]"><span>Subtotal</span><span className="text-[#222222]">{formatPrice(subtotal)}</span></div>
                <div className="flex justify-between font-bold text-base pt-2 border-t border-[#ebebeb] text-[#222222]"><span>Total</span><span>{formatPrice(total)}</span></div>
              </div>
            </div>
          </div>
        </div>
