...
export default function CheckoutPage({ cartItems }: { cartItems: CartItem[] }) {
  ...
  async function handleCheckout(e: React.FormEvent) {
    ...
    try {
      ...
      // Initiate M-Pesa STK push for the first order (single-item checkout)
      // For multi-item, the first order represents the total
      if (orderIds.length > 0) {
        // In production, call the M-Pesa STK push API here
        // For now, redirect to order confirmation
      }

      setStep("success");

      // Redirect to first order after 2 seconds
      if (orderIds.length > 0) {
        setTimeout(() => {
