  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      <div className="bg-gradient-to-br from-emerald-700 to-emerald-900 text-white rounded-2xl p-8 mb-8">
        ...
        <p className="mt-2 text-emerald-100">The trusted P2P marketplace. Secure payments, verified sellers.</p>
        <form className="mt-4 max-w-xl flex gap-2" method="get">
          ...
          />
          <button type="submit" className="px-6 py-2 bg-white text-emerald-700 rounded-lg font-medium hover:bg-emerald-50">
            Search
          </button>
        </form>
      ...
      <div className="flex gap-8">
        <aside className="w-56 shrink-0 hidden md:block">
          ...
          <div className="space-y-1 mb-6">
            <Link href={`/?${fp.toString()}`} className={`block px-3 py-2 rounded-lg text-sm ${!params.category ? "bg-emerald-50 text-emerald-700 font-medium" : "hover:bg-gray-50"}`}>
              All Categories
            </Link>
            {CATEGORIES.map((cat) => (
              <Link
                ...
                className={`block px-3 py-2 rounded-lg text-sm ${params.category === cat.slug ? "bg-emerald-50 text-emerald-700 font-medium" : "hover:bg-gray-50"}`}
              >
                {cat.icon} {cat.name}
              </Link>
            ))}
          ...
          <div className="space-y-1 mb-6">
            <Link href={`/?${fp.toString().replace(/condition=[^&]*/g, "")}`} className={`block px-3 py-2 rounded-lg text-sm ${!params.condition ? "bg-emerald-50 text-emerald-700 font-medium" : "hover:bg-gray-50"}`}>
              All Conditions
            </Link>
            {CONDITIONS.map((c) => {
              ...
              return (
                <Link key={c.value} href={`/?${condParams.toString()}`} className={`block px-3 py-2 rounded-lg text-sm ${params.condition === c.value ? "bg-emerald-50 text-emerald-700 font-medium" : "hover:bg-gray-50"}`}>
                  {c.label}
                </Link>
              );
            })}
        ...
        <div className="flex-1">
          <div className="flex items-center justify-between mb-4 gap-4 flex-wrap">
            <p className="text-gray-500 text-sm">{totalListings} listing{totalListings !== 1 ? "s" : ""} found</p>
          ...
          {listings && listings.length > 0 ? (
            <>
              ...
              {totalPages > 1 && (
                <div className="flex items-center justify-center gap-2 mt-8">
                  {page > 1 && (
                    <Link href={`/?page=${page - 1}${filterStr ? `&${filterStr}` : ""}`} className="px-3 py-1.5 border rounded-lg text-sm hover:bg-gray-50">Prev</Link>
                  )}
                  {Array.from({ length: Math.min(totalPages, 5) }, (_, i) => {
                    const p = Math.max(1, Math.min(page - 2, totalPages - 4)) + i;
                    if (p > totalPages || p < 1) return null;
                    return (
                      <Link key={p} href={`/?page=${p}${filterStr ? `&${filterStr}` : ""}`} className={`w-8 h-8 rounded-lg text-sm flex items-center justify-center ${p === page ? "bg-emerald-600 text-white" : "border hover:bg-gray-50"}`}>
                        {p}
                      </Link>
                    );
                  })}
                  {page < totalPages && (
                    <Link href={`/?page=${page + 1}${filterStr ? `&${filterStr}` : ""}`} className="px-3 py-1.5 border rounded-lg text-sm hover:bg-gray-50">Next</Link>
                  )}
                </div>
              )}
            </>
            ...
            <div className="text-center py-20 text-gray-400">
              ...
              <Link href="/sell" className="inline-block mt-4 px-4 py-2 bg-emerald-600 text-white rounded-lg text-sm font-medium hover:bg-emerald-700">
